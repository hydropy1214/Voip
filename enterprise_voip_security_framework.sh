...
phase1b_service_enumeration() {
    ...
    while IFS= read -r ip; do
        {
            # RFC 3261 §4.1 - SIP OPTIONS used as keep-alive / capability probe
            # Minimal compliant packet: required headers are Via, To, From, Call-ID, CSeq
            (
                echo -e "OPTIONS sip:$ip:5060 SIP/2.0\r\nVia: SIP/2.0/UDP scanner\r\nTo: <sip:$ip>\r\nFrom: <sip:scanner>\r\nCall-ID: scanner\r\nCSeq: 1 OPTIONS\r\n\r\n"
                sleep 1
            ) | nc -q 1 -w 2 "$ip" 5060 >> "$enum_output" 2>/dev/null \
                || log_debug "SIP OPTIONS probe failed for $ip:5060"
...
extract_sip_banner() {
    ...
    # RFC 3261 §4.1 - minimal OPTIONS request to probe server capabilities.
    # Required headers: Via, To, From, Call-ID, CSeq (RFC 3261 §20).
    # timeout wraps the full pipeline; bash -c avoids subshell-in-pipeline syntax error.
    local response
    ...
    " | nc -w 2 "$ip" 5060 2>/dev/null) \
        || log_debug "SIP banner grab failed for $ip:5060"
    
    # Extract Server header (RFC 3261 §20.35)
